...
/**
 * This examples show some extra queries and most notably the "OR WHERE" clause
 * which is useful to concatenate two conjunctive queries. We use the OWL wine
 * ontology for demonstration and the built-in StructuralReasoner as sample
 * reasoning system. In case you use any other reasoning engine make sure you
 * have the respective jars within your classpath (note that you have to provide
 * the resp. ReasonerFactory in this case).
 *
 * @author Mario Volke
...
public class Example_Extended {
	...
	public static void main(String[] args) {
		try {
			...
			// Optionally let the reasoner compute the most relevant inferences
			// in advance
			...
			// Some queries which demonstrate more sophisticated language
			// constructs of SPARQL-DL
			...
			// A complicated way to retrieve all individuals. Note that the
			...
			// A query returning pairs of results, namely all sources and
			// fillers of yearValue
			processQuery("PREFIX wine: <http://www.w3.org/TR/2003/PR-owl-guide-20031209/wine#>\n"
			...
			processQuery("SELECT ?i WHERE {" + "ObjectProperty(?i) " + "} OR WHERE {" + "Class(?i)" + "}");
